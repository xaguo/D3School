 ...
 */
var bWMTSImageryProvider = function bWMTSImageryProvider(description) {
...
function buildImageUrl(imageryProvider,  x, y, level) {
   ...
   /* var zoom = level + 1;
    ...
    var url = imageryProvider._url ;
	/*
   url += '?SERVICE=WMTS&REQUEST=GetTile&VERSION=1.0.0&LAYER=eia&STYLE=default&TILEMATRIXSET=w&';
    //url += '&LAYER=' + '';
   // url += '&STYLE=' + '';
    //url += '&TILEMATRIXSET=' + 'YN_SAT';
    url += '&TILEMATRIX=' + zoom ;
    url += '&TILEROW=' + y ;
    url += '&TILECOL=' + x ;
    url += '&FORMAT=tiles';// + imageryProvider._fileExtension;*/
	url+=zoom+"/";
	url+=y+"/";
	url+=x;
...
Cesium.defineProperties(bWMTSImageryProvider.prototype, {
    url : {
        get : function() {
            return this._url;
        }
...
bWMTSImageryProvider.prototype.getTileCredits = function(x, y, level) {
    return undefined;
};

bWMTSImageryProvider.prototype.requestImage = function(x, y, level) {
    if (!this._ready) {
